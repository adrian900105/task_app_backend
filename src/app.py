...
def register():
    if request.method == 'POST':
        ...
        if users_collection.find_one({'email':email}):
            return get_response('Użytkownik o podanym adresie już istnieje',False,500)

        # Sprawdzenie hasła za pomocą regular expression
        if re.match(password_pattern, password) is None:
            return get_response('Hasło musi zawierać małą, dużą literę, znak specjalny, cyfrę i minimum 5 znaków',False,500)      
...
def get_users():
    ...
    # return get_response('Wyświetlono użytkowników',True,200,users)
